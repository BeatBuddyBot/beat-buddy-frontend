...
const SongsTable = ({ playlist, setPlaylist }) => {
  ...
  const handleRemoveSong = (params) => {
    ...
    ApiService.deleteSong(idToRemove)
      .then(() => {
        setPlaylist((prev) => ({
          ...prev,
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <Button
          ...
          color="neutral"
          size="small"
          onClick={() => handleRemoveSong(params)}
        >
          <ClearOutlinedIcon />
        </Button>
      ),
  ...
  function onRowDragEnd(e) {
    ApiService.patchSong(e.node.data.id, {
      position: e.node.rowIndex,
    })
      .then(() => {
        setPlaylist((prev) => {
          const songs = [...prev.songs];
          const draggedSongIndex = songs.findIndex(
            (song) => song.id === e.node.data.id
          );
